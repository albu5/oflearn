...
def getBatch(BS, img1_list, img2_list, flow_list, HEIGHT, WIDTH, FLO_MAX):
	N = len(img2_list)
...
# I1,I2,F = getBatch(BS, img1_list, img2_list, flow_list, HEIGHT, WIDTH)
# print(I1.shape,F.shape)
while True:
	I1,I2,F = getBatch(BS, img1_list, img2_list, flow_list, HEIGHT, WIDTH, FLO_MAX)
	print('max gt flow is', np.max(F))

	if niter%DISP == 0:
		flow = np.squeeze(net.flow2.eval(feed_dict={net.inp1: np.expand_dims(I1[0][:][:][:],0), net.inp2: np.expand_dims(I2[0][:][:][:],0)}))
		pool5_1 = np.squeeze(net.pool5_1.eval(feed_dict={net.inp1: np.expand_dims(I1[0][:][:][:],0), net.inp2: np.expand_dims(I2[0][:][:][:],0)}))
